...
def max_circle(photo_path):
    ...
    plt.figure()
    plt.imshow(img_gray)
    # 创建线程组 目的是因为主线程是需要等待所有线程结束 才进行执行 我们并不能确定有多少个线程在执行 所以要创建相应的数组 存放线程
    threads = []
    # 用多线程来解决速度缓慢的问题  同时要考虑到线程同步的过程  我们要等所有线程结束 我们才能显示相应的圆
    for c in contours:
        thread = MyThread(c, plot_x)
        thread.start()
...
class MyThread(threading.Thread):
    def __init__(self, c, plot_x):
        threading.Thread.__init__(self)
        self.c = c
        self.plot_x = plot_x

    def run(self):
        draw_circle(self.c, self.plot_x)
